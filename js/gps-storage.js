...
(function (global) {
   ...
   *
   *  ИСПРАВЛЕНИЕ (баг с потерей пробега при блокировке телефона):
   *  gps-tracker.js перед вызовом этой функции сам решает, продолжать ли
   *  старую сессию (передаёт её же sessionId, если по этому номеру уже
   *  есть активная) или начать новую (генерирует новый sessionId). Раньше
   *  startSession() это игнорировала и ВСЕГДА создавала состояние с нуля
   *  (distanceKm: 0) — из-за этого при любом повторном вызове start() для
   *  уже активной сессии (типичный случай: мобильный браузер выгрузил
   *  страницу из памяти при блокировке экрана, при разблокировке она
   *  загрузилась заново и driver.html снова вызвал startDriverGps_())
   *  весь накопленный, ещё не отправленный на сервер пробег молча
   *  обнулялся — и не сохранялся в очередь, потому что stopSession() для
   *  неё не вызывался. Теперь: если для этого номера уже есть активная
   *  сессия с ТЕМ ЖЕ sessionId — просто продолжаем её (сохраняя
   *  distanceKm/syncedDistanceKm/последнюю точку), а не создаём заново. */
  function startSession(plate, sessionId, deviceToken) {
    ...
    var existing = getState(plate);
    if (existing && existing.status === 'active' && existing.sessionId === sessionId) {
      // Это повторный старт ТОЙ ЖЕ сессии (например, после перезагрузки
      // страницы) — продолжаем её, не теряя уже накопленный пробег.
      existing.deviceToken = deviceToken || existing.deviceToken || '';
      setState(plate, existing);
      return existing;
    }

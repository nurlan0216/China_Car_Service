...
Если фронтенд получает `unknown payload`, это означает, что используется старая опубликованная версия Apps Script, в которой ещё нет `addStory`. Новый `to.html` теперь показывает это прямо, а не скрывает под общей ошибкой сети.
